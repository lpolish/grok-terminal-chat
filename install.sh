...
setup_virtualenv() {
    ...
    debug_log "Creating Python virtual environment"
...
if [ ! -t 0 ]; then
    exec bash -c "$(declare -f debug_log fail safe_mkdir install_system_deps \
                   create_python_script create_grok_script setup_virtualenv main); main"
else
    main
fi
